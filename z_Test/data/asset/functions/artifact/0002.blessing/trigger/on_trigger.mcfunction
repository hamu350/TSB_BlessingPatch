...
# エラー値
    execute unless score @s 02.Trigger matches 1..199 run function lib:message/invalid_operation
    execute unless score @s 02.Trigger matches 1..199 run return run function asset:artifact/0002.blessing/trigger/show_trigger_chat
# Args.ID = f"{Trigger:>2}"
# Args.UpdatePath = bls_patch: Blessing.{ID}.UpdatePath
    ...
    execute if score @s 02.Trigger matches 1..99 run function asset:artifact/0002.blessing/trigger/get_id.m with storage bls_patch: Args
...
# [MAX] 101~199
    execute if score @s 02.Trigger matches 101..199 run function asset:artifact/0002.blessing/trigger/max/
# リセット
